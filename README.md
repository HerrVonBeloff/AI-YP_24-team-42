...
Важно отметить, что по сравнению с более простой ранней реализацией IS и FID (ранее мы не использовали CLIP) стали значительно хуже ($\mathrm{IS}\approx0.20$, $\mathrm{FID}\approx$ 3.3). Такое постоянство метрик дополнительно показывает, что своими улучшениями мы не затронули главную первопричину низкого качества модели.
