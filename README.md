...
Все изменения были введены постепенно после увеличения размера изображения. Ввиду ограничения по времени и ресурсам, проводилось текстовое обучение на всём датасете в течение 2-10 эпох (случаи 2 эпох были достаточны в случае очевидных правалов обучения или очень долгих расчётов). Было замечено, что дискриминатор почти сразу становился сильнее генератора, генератор в свою очередь застревал на некоем паттерне, который был склонен повторять для разных изображений (mode collapse). По этой причине были попытки учесть разнообразие генераций, ввод дропаута, увеличение циклов обучения для генератора и т.д. Однако ни один из подходов никак не мог решить этой проблемы. Потери для генератора были склонны расти, а метрики качества обычно выходили на подобие плато и колебались вокруг него. Попытки изменить обработку текстовых данных также не привели к значительным улучшениям.
...
Метрики, как правило, выходили на плато в диапозоне одних и тех же значений на протяжении всех экспериментов. IS колебаллся в районе 1.8, CLIP колебался на укровне 0.18, FID колебался чуть шире: от 1000 до 1200. Причем метрики IS и FID (ранее мы не использовали CLIP) значительно хуже, чем значения метрик ранее ($\mathrm{IS}\approx0.20$, $\mathrm{FID}\approx$ 3.3). Такое постоянство метрик дополнительно показывает, что своими улучшениями мы не затронули главную первопричину низкого качества модели.

Предположений о том, как это исправить, ещё много. Вероятно, разрешение выходного изображения слишком большое для нашего датасета (наболее частое разрешение изображений в нём $180\times180$). Возможно, нормализация по батчам негативно влияет на сетку текстовых признаков. Также возможно, но маловероятно, что мы слишком усложнили дискриминатор, из-за чего он сильно вырывается вперёд и делает обучение генератора невозможным. Возможно, следует получше подобрать константу $\lambda$ для CLIP-guided функции потерь, чтобы увеличить вес смысла текста в ней. Тем не менее, нам не хватит времени проверить все гипотезы. Стоит также отметить, что после усложнения функции потерь очень сильно возросло время обучения (даже на GPU на Kaggle - на отведённые 12 часов работы обучение длилось всего 2(!) эпохи). Также мы столкнулись со сложностями интерпретации сложных функций потерь по сравнению с их простыми аналогами - поведение этих функций и их допустимые значения довольно сильно отличаются.
