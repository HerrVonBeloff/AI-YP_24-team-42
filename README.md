...
Важно отметить, что по сравнению с более простой ранней реализацией IS и FID (ранее мы не использовали CLIP) стали значительно хуже (ранее $\mathrm{IS}\approx0.20$, $\mathrm{FID}\approx$ 3.3). Такое постоянство метрик дополнительно показывает, что своими улучшениями мы не затронули главную первопричину низкого качества модели.
...
Модель использует технику LoRA (Low-Rank Adaptation) для тонкой настройки, что позволяет обучать только небольшую часть параметров, не изменяя оригинальные веса. Обучение состояло из следующих этапов:

* Кэширование латентов через VAE
* Токенизация описаний и генерация text embeddings
* Добавление шума в латенты по диффузионному расписанию
